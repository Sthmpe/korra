...
// 2. CRYPTO HELPER (Paystack uses HMAC SHA-512)
async function verifyPaystackHash(bodyText: string, signature: string, secret: string): Promise<boolean> {
  ...
  const hashHex = Array.from(new Uint8Array(signatureBytes)).map(b => b.toString(16).padStart(2, '0')).join('');
...
serve(async (req) => {
  ...
  try {
    ...
    // A. Security Check
    if (!signature || !PAYSTACK_SECRET_KEY) return new Response("Unauthorized", { status: 401 });

    const isValid = await verifyPaystackHash(bodyText, signature, PAYSTACK_SECRET_KEY);
    ...
    // =========================================================================
    // 🚦 ROUTE 1: CUSTOMER DEPOSITS (FUND WALLET) -> Paystack: charge.success
    // =========================================================================
    if (event === "charge.success") {
      const transactionId = data.reference;
      const customerCode = data.customer?.customer_code;
      const amountPaid = data.amount / 100; // ⚠️ Paystack sends kobo, convert to Naira
      const formattedAmount = new Intl.NumberFormat('en-NG', { style: 'currency', currency: 'NGN' }).format(amountPaid);

      console.log(`Processing ${transactionId} for Customer Code ${customerCode}`);

      if (!customerCode) return new Response(JSON.stringify({ status: "ignored" }), { status: 200 });

      // 1. LOOKUP CUSTOMER UID BY PAYSTACK CODE
      const customersQuery = await db.collection('customers').where('paystack.customerCode', '==', customerCode).limit(1).get();
      if (customersQuery.empty) {
        console.log(`⚠️ Customer not found for code ${customerCode}. Skipping.`);
        return new Response(JSON.stringify({ status: "ignored" }), { status: 200 });
      }
      const uid = customersQuery.docs[0].id;
      ...
      await db.runTransaction(async (t) => {
        ...
        const currentBalance = userData?.paystack?.availableBalance || 0.00; // Updated to paystack object
        ...
        // Write A: Ledger Entry (Matches TransactionModel EXACTLY)
        t.set(ledgerRef, {
          id: `${transactionId}`,
          customerId: uid,
          amount: amountPaid,
        ...
        // Write B: Update Balance (Updated to paystack.availableBalance)
        t.update(userRef, {
          "paystack.availableBalance": admin.firestore.FieldValue.increment(amountPaid)
        });

        // Write C: In-App Notification (Exact Schema)
    ...
    // =========================================================================
    // 🚦 ROUTE 2: VENDOR PAYOUTS (DISBURSEMENTS) -> Paystack: transfer.*
    // =========================================================================
    if (event === "transfer.success" || event === "transfer.failed" || event === "transfer.reversed") {
      
      const reference = data.reference; // e.g., PAYOUT|LTT8F140|EYT2
      
      // 1. LOOKUP THE MAPPING PHONEBOOK
      const mappingDoc = await db.collection('monnify_mappings').doc(reference).get(); // Kept exactly as you named it
      ...
      await db.runTransaction(async (t) => {
        ...
        if (ledgerData?.status === 'success' || ledgerData?.status === 'failed' || ledgerData?.status === 'reversed') {
        ...
        // WRITES: FAILED OR REVERSED CASE
          ...
          t.update(ledgerRef, { 
            ...
            gatewayResponse: data.reason || "Bank reversal" 
          });
          ...
          t.set(refundRef, {
            ...
            description: `Refund for failed payout (${event})`,
            createdAt: timestamp
          });
