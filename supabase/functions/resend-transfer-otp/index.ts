// supabase/functions/resend-transfer-otp/index.ts
...
async function resendOtp(body: any) {
  const { reference } = body ?? {};
  if (!reference) {
    return new Response(
      JSON.stringify({ ok: false, status: "FAILED", message: "Missing reference" }),
  ...
  const res = await fetch(`${BASE_URL}/api/v2/disbursements/single/resend-otp`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok || !data.requestSuccessful) {
    return new Response(
      JSON.stringify({
        ok: false,
        status: "FAILED",
        message: data.responseMessage || "Resend OTP failed",
      }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  }
  ...
  return new Response(
    JSON.stringify({
      ok: true,
      status: "SUCCESS", // Resend OTP is not final, so keep it pending
...
serve(async (req) => {
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ ok: false, status: "FAILED", message: "Only POST allowed" }),
