...
class _VendorReviewsBodyState extends State<VendorReviewsBody> {
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text(
                "Seed Demo Reviews & Orders (25+ items)",
  ...
  Future<void> _seedMockData(BuildContext context) async {
    ...
    final names = [
      "Sarah Jenkins", "Michael Chen", "Amara Okafor", "David Smith", "Blessing Temi",
      "Liam Carter", "Olivia Vance", "Noah Albright", "Emma Stone", "Oliver Brooks",
      "Ava Sterling", "Elijah Cross", "Charlotte Hayes", "William Foster", "Sophia Mercer",
      "James Thorne", "Amelia Vance", "Benjamin Croft", "Isabella Grey", "Lucas Thorne",
      "Mia Fletcher", "Henry Gable", "Evelyn Reed", "Alexander Vance", "Harper Knight"
    ];

    final comments = [
      "Outstanding quality! Will buy again.",
      "Incredible service, very fast shipping.",
      "Extremely satisfied with the purchase.",
      "Perfect product, works exactly as described.",
      "Very polite seller, great communication.",
      "Good product, took a bit long to arrive.",
      "Nice packaging, overall good value.",
      "Happy with the quality, thanks.",
      "Solid purchase, would recommend.",
      "Decent service, product is good.",
      "Average quality, matches the price.",
      "Okay experience, shipping was delayed.",
    ...
    // 1. Seed 25 Reviews
    final reviewsRef = firestore.collection('vendors').doc(widget.vendorId).collection('reviews');
    for (int i = 0; i < 25; i++) {
      final name = names[i % names.length];
      final double rating = (i % 5 == 0) ? 3.0 : ((i % 3 == 0) ? 4.0 : 5.0);
      final comment = comments[i % comments.length];
      final doc = reviewsRef.doc();
      batch.set(doc, {
        'customerId': 'mock_cust_$i',
        'customerName': name,
        'rating': rating,
        'comment': comment,
        'createdAt': Timestamp.fromDate(DateTime.now().subtract(Duration(hours: i * 3))),
        'isMock': true,
      });
    }

    // 2. Seed 25 Outright Orders (21 pending, 2 readyToDeliver, 1 delivered, 1 cancelled)
    ...
    for (int i = 0; i < 25; i++) {
      ...
      final doc = ordersRef.doc();
      batch.set(doc, {
        'vendorId': widget.vendorId,
        'customerId': 'mock_cust_$i',
        'customerName': name,
        'customerPhone': '+23480${3000000 + i}',
        'totalAmount': total,
        'status': status,
        'createdAt': Timestamp.fromDate(DateTime.now().subtract(Duration(hours: i * 2))),
        if (status == 'delivered') 'deliveredAt': Timestamp.fromDate(DateTime.now().subtract(const Duration(hours: 12))),
        if (status == 'cancelled') 'cancelledAt': Timestamp.fromDate(DateTime.now().subtract(const Duration(hours: 24))),
        'isMock': true,
        'items': items,
      });
    }
    ...
    try {
      await batch.commit();
      if (context.mounted) {
        showAppSnackbar("Successfully seeded 25 reviews and 25 orders!", SnackbarType.success);
      }
      _onRefresh(); // Reload list to display first page of seeded reviews
    } catch (e) {
      if (context.mounted) {
        showAppSnackbar("Failed to seed mock data: $e", SnackbarType.error);
      }
    }
  }
