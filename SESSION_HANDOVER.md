...
## 20 July 2026 - Campaign push notification daily cap (SERVER-ONLY, no app rebuild needed)
David confirmed and asked to implement: customer-side cap of 3 pushed campaign notifications per day, combined across all followed stores, one slot per DISTINCT store (a store can never claim a 2nd slot the same day even if it broadcasts twice; unused slots are not backfilled by a repeat store).
- supabase/functions/campaign-broadcast/index.ts: NEW claimPushSlot(customerId, vendorId, today) - transaction on customers/{uid}/campaignPushDaily/{yyyy-MM-dd}.vendorIds (array of distinct vendorIds pushed today, capped at 3). Returns false if this vendorId is already in the array (store already used its slot) or the array already has 3 entries.
- Fan-out loop: for every eligible (non-muted) follower, the in-app notification write is UNCHANGED and always happens. The push (fcmToken added to the multicast batch) now only happens if claimPushSlot returns true. Response payload gains `capped` count.
- Purely per-customer: their own my_vendors follow list decides who's reached at all (unsaved stores never notify that customer, muted stores skip both in-app+push); their own campaignPushDaily doc decides their own slot usage, independent of every other customer.
- No app change needed - customer app already just renders whatever in-app notifications/pushes arrive.
DAVID TODO: deploy campaign-broadcast.

## 25 July 2026 - Play Store rejection fix: email login returned for review access (customer + merchant login screen)
Google rejected the com.korra.shop.live update ("Login credentials restricted by device authentication") - the login screen only had Continue with Google, so the only credentials we could give reviewers was a Gmail account, and Google account security blocks sign-in from the reviewer device. Fix per David: users still sign UP with Google only; email/password is LOGIN-ONLY, as a subtle secondary option.
- NEW lib/presentation/auth/role_login/widgets/email_login_section.dart: collapsed "or sign in with email" link under the Google button; expands to email + password + Sign in. Wired to the existing RoleLoginBloc (SubmitPressed etc. - that logic was never removed, only the UI was). No signup path, no forgot-password.
- role_login_screen.dart: renders EmailLoginSection under the Google button (shared screen, so merchant flavor gets it too - same rejection risk there).
- NEW scripts/set_demo_password.mjs: one-off admin script - adds a password to an EXISTING Firebase user via admin.updateUser(uid, {password}). Plan: David signs up a throwaway-Gmail demo account in the app (full onboarding), runs the script to attach a password, gives Google that email+password.
DAVID TODO: (1) create demo account with Google in the live app + full onboarding; (2) run scripts/set_demo_password.mjs with the service-account JSON; (3) build + upload new AAB for BOTH flavors ideally; (4) Play Console -> App content -> App access: new credentials + note "tap 'or sign in with email', no OTP at login"; (5) Publishing overview -> send for review.
